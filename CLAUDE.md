...
```bash
bash apply-canary-husky-user-gitconfig.sh <canary-repo-경로>
```

스크립트 동작 순서 (4단계):
1. `~/.gitconfig`에 `[includeIf "gitdir/i:<canary-repo>/"]` 항목을 추가하여 canary 저장소 안에서만 `core.hooksPath = .husky`가 적용되도록 합니다.
