...
`git commit` 시 staged된 `*.cs` 파일에 대해 아래 두 가지 포매팅이 자동 실행됩니다.

- `dotnet format whitespace` — 들여쓰기·공백 정규화
- `dotnet format style` — 불필요한 using 제거(IDE0005), file-scoped namespace 변환(IDE0161)
...
```bash
bash apply-canary-husky-user-gitconfig.sh <프로젝트-경로>
```
...
```bash
# 예시
bash apply-canary-husky-user-gitconfig.sh C:/Users/me/Projects/MyGame
bash apply-canary-husky-user-gitconfig.sh /c/Users/me/Projects/MyGame
```
...
설치 파일이 이미 존재하면 건너뜁니다. 재설치가 필요하면 해당 파일을 삭제 후 재실행.
